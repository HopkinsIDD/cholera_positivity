...
// using a meta-analysis across >100 studies, with covariates, random effect by study.
...
model {
  // proportion positive
  // culture
  target += binomial_lpmf(num_pos[culture_id_long] | num_test[culture_id_long], p[culture_id_wide]);
  // pcr
  target += binomial_lpmf(num_pos[pcr_id_long] | num_test[pcr_id_long], p[pcr_id_wide]);
  // rdt
  target += binomial_lpmf(num_pos[rdt_id_long] | num_test[rdt_id_long], p[rdt_id_wide]);
